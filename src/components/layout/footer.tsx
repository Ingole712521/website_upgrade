...
export function Footer() {
  return (
    <motion.footer
      ...
      className="border-t border-foreground/15 bg-foreground text-background px-5 py-14 sm:px-8"
    >
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-10 lg:grid-cols-[1.3fr_1fr_1fr_1fr_1fr]">
          <motion.div variants={staggerItem}>
            <Link to="/" aria-label="zCon Solutions home" className="inline-block brightness-0 invert">
            ...
            <p className="mt-5 font-mono text-[0.65rem] uppercase tracking-[0.18em] text-background/45">
          ...
          {columns.map((column) => (
            <motion.div key={column.title} variants={staggerItem}>
              <h3 className="font-mono text-[0.65rem] font-medium uppercase tracking-[0.18em] text-primary">
                {column.title}
              </h3>
        ...
        <motion.div
          ...
          className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-background/15 pt-8 sm:flex-row"
        >
          <p className="text-xs text-background/45">
            © {new Date().getFullYear()} zCon Solutions. All rights reserved.
          </p>
          <div className="flex items-center gap-5 font-mono text-[0.65rem] uppercase tracking-[0.14em] text-background/45">
            <span>ISO 9001</span>
            <span>ISO 27001</span>
